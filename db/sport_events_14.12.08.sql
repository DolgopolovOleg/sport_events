...
insert  into `user_auth`(`_id`,`user_id`,`login`,`password`,`enabled`) values (1,1,'user1','user1',1),(2,2,'user2','user2',0);
...
CREATE TABLE `user_role` (
  ...
  KEY `role_id` (`role_id`),
  CONSTRAINT `user_role_id` FOREIGN KEY (`user_id`) REFERENCES `user` (`_id`),
  CONSTRAINT `role_id` FOREIGN KEY (`role_id`) REFERENCES `role` (`_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

/*Data for the table `user_role` */

insert  into `user_role`(`user_id`,`role_id`) values (1,2),(1,2),(2,2);
